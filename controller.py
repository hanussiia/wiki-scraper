...
class Controller:
    def __init__(self, parser:ParserHTML,language: str='en', args:dict=None):
        self.args = args
        self.parser = parser
    ...
    def what_task(self):
        if self.args.summary:
            ...
            self.table(self.args.table, self.args.number, self.args.first_row_is_header)
        elif self.args.count_words:
            self.count_words(self.args.count_words)
        elif self.args.analyze_relative:
            self.analyze_relative(mode=self.args.mode, n=self.args.count, chart_path=self.args.chart)
        elif self.args.auto_count_words:
            self.auto_count_words(phrase=self.args.auto_count_words, depth=self.args.depth, t=self.args.wait)
    ...
    def analyze_relative(self, mode, n, chart_path):
        self.word_analyzer.analyze_relative_word_frequency(mode=mode, n=n, chart_path=chart_path)
